...
(
  async () => {
    try {
      ...
      if (
        serversResponse.servers.includes('Inspection')
        ...
        const dbCleanResponse = await prompts(
          {
            type: 'confirm',
            name: 'dbClean',
            message: 'Would you like to drop the existing database tables?',
            initial: false,
          },
        );

        if (dbCleanResponse.dbClean) {
          const dbCleanArgs = [
            path.join(
              __dirname,
              'node_modules',
          ...
          if (configFile) {
            dbCleanArgs.push(configFile);
          }

          console.log('Cleaning the database...');
          const cp = spawnSync('node', dbCleanArgs);
          if (cp.status === 0) {
            console.log('Cleaned the database successfully');
          } else {
